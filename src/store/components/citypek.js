...
const actions = {
  find:  ({commit}, payload) => {
    return new Promise((resolve, reject) => {
      Http.get(Config.apiUrl + 'citypek?' + ((payload != null && payload.query != null) ? 'query=' + payload.query : '') + ((payload != null && payload.page != null) ? 'page='+payload.page : ''))
        .then(resp => {
  ...
  delete: ({commit, state}, id) => {
    return new Promise((resolve, reject) => {
      Http.get(Config.apiUrl + 'citypek/delete?id='+ id)
        .then((r) => {
          var arr = []
  ...
  save:  ({commit}, model )=> {
    return new Promise((resolve, reject) => {
      if (model.id != null && model.id != 0) {
        Http.post(Config.apiUrl + 'citypek/update', model)
          .then(resp => {
            resolve(resp)
          })
          .catch(err => {
        ...
        Http.post(Config.apiUrl + 'citypek/create', model)
          .then(resp => {
            resolve(resp)
          })
          .catch(err => {
